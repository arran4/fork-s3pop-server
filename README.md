...
| `S3POP_CONFIG` | Path to a custom JSON configuration file (defaults to `/etc/s3pop/server-config.json` in Docker if not overridden, or `./server-config.json` locally). | `/etc/s3pop3/server-config.json` |
